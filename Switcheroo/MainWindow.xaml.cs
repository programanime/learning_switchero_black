...
namespace Switcheroo
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public static readonly RoutedUICommand RenameWindowCommand = new RoutedUICommand();
        ...
        private void LoadData(InitialFocus focus)
        {
            ...
            {
                window.FormattedTitle = new XamlHighlighter().Highlight(new[] { new StringPart(Prompt.getWindowTitle(window.AppWindow.Title)) });
        ...
        private void AltTabPressed(object sender, AltTabHookEventArgs e)
        {
            ...
            if (Visibility != Visibility.Visible)
            {
                ...
                if (Settings.Default.AutoSwitch && e != null && !e.CtrlDown)
                {
        ...
        private void ActivateAndFocusMainWindowTwo() 
        {
            // What happens below looks a bit weird, but for Switcheroo to get focus when using the Alt+Tab hook,
            // it is needed to simulate an Alt keypress will bring Switcheroo to the foreground. Otherwise Switcheroo
            // will become the foreground window, but the previous window will retain focus, and receive keep getting
        ...
        private async void RenameWindow(object sender, ExecutedRoutedEventArgs e)
        {
            var windows = lb.SelectedItems.Cast<AppWindowViewModel>().ToList();
            foreach (var win in windows)
            {
                var resultad = Prompt.ShowDialog("", "");
                if (!string.IsNullOrEmpty(resultad))
                {
                    Prompt.setWindowTitle(win.WindowTitle, resultad);
                }
            }
